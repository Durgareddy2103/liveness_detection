...
=======================================================
Input-resolution routing (this revision):
  - Haar face detection + OpenCV occlusion gate run on the RESIZED frame
    (capped at MAX_IMAGE_SIDE). The CV gate's phone/edge/skin thresholds are
    calibrated on 480px, so it MUST stay on the resized frame.
  - Silent-Face and kprokofi run on the ORIGINAL full-res frame:
      * Silent-Face does its own MTCNN bbox internally, so it just gets the
        original image.
      * kprokofi crops with the Haar bbox, which is in resized coordinates, so
        the bbox is scaled up to original-image coordinates via _scale_bbox()
        before cropping from the full-res frame.
...
log.info("Image cap | max_side=%d px (Haar+CV gate only; liveness models use original)",
         MAX_IMAGE_SIDE)
...
def _check_phone_obstruction(
    ...
    NOTE: This runs as part of the CV occlusion gate, which operates on the
    RESIZED frame. Its thresholds are calibrated on 480px — do not feed it the
    original full-res frame.
    ...
    properties that a beard or shirt collar does NOT:
      ...
      3. NON-SKIN BELOW — A phone body is not skin-toned regardless of colour.
         → Measured: 1 - skin(below). Colour-space agnostic.

    Score = (skin_drop × boundary_sharpness × non_skin_below) / divisor
    ...
    Only searches PHONE_ZONE_MIN (0.40) to PHONE_ZONE_MAX (0.82) of face height:
      - Below 0.40: glasses-frame zone; excluded.
      - Above 0.82: chin-to-shirt-collar transition; excluded.

    Args:
        image : RESIZED BGR frame (capped at MAX_IMAGE_SIDE)
        bbox  : (x, y, w, h) face bounding box from Haar detector (resized coords)
...
def _run_cv_visibility(
    ...
    Operates on the RESIZED frame (image) with the Haar bbox in resized coords.
    Pass 3: dark-object
    Pass 4: wide-horizontal-edge
...
def _scale_bbox(
    ...
    """
    Map a Haar bbox from the RESIZED image back onto the ORIGINAL frame.

    Scales the corners (x1,y1,x2,y2) rather than (x,w)/(y,h) independently,
    which avoids rounding drift between the origin and the far edge. Result is
    clamped to the original image bounds. When factor == 1.0 (no resize
    happened) this is an identity transform modulo clamping.
    """
    x, y, w, h = bbox
    x1 = max(0, min(int(round(x * factor)),         max_w))
    y1 = max(0, min(int(round(y * factor)),         max_h))
    x2 = max(0, min(int(round((x + w) * factor)),   max_w))
    y2 = max(0, min(int(round((y + h) * factor)),   max_h))
...
def _run_kprokofi(
    ...
    """
    Runs kprokofi MN3 on the ORIGINAL full-res frame.
    `bbox` must already be expressed in original-image coordinates
    (use _scale_bbox to convert from the resized Haar bbox).
    """
...
def _run_silent_face(image: np.ndarray, rid: str = "") -> Dict[str, Any]:
    """
    Runs Silent-Face on the ORIGINAL full-res frame. It does its own MTCNN
    bbox detection internally, so it does not consume the Haar bbox.
    """
...
def _decode_and_resize_image(
...
) -> Tuple[Optional[np.ndarray], Optional[np.ndarray], float, int, int, float]:
    """
    Decode the upload once and produce both frames used downstream.

    Returns:
        (orig_image, small_image, resize_ms, orig_w, orig_h, scale_to_orig)
          orig_image    : full-res decoded BGR frame (fed to the liveness models)
          small_image   : frame capped at MAX_IMAGE_SIDE (fed to Haar + CV gate);
                          is the SAME object as orig_image when no resize was needed
          resize_ms     : time spent resizing, 0.0 if not resized
          orig_w/orig_h : original dimensions
          scale_to_orig : multiply a small-image coordinate by this to map it
                          onto orig_image. 1.0 when no resize happened.

    On decode failure returns (None, None, 0.0, 0, 0, 1.0).
    """
    ...
    if max_side > MAX_IMAGE_SIDE:
        ...
        scale_to_orig = orig_w / float(new_w)
        log.info("[%s][liveness] resized %dx%d -> %dx%d in %.1fms "
                 "(scale_to_orig=%.4f; Haar+CV use small, models use orig)",
                 rid, orig_w, orig_h, new_w, new_h, resize_ms, scale_to_orig)

    return image, small_image, resize_ms, orig_w, orig_h, scale_to_orig
...
def _run_liveness(image_bytes: bytes, rid: str = "") -> Dict[str, Any]:
    ...
    orig_image, small_image, resize_ms, orig_w, orig_h, scale_to_orig = \
        _decode_and_resize_image(image_bytes, rid)
    ...
    # Silent-Face: full-res ORIGINAL (does its own MTCNN bbox internally)
    t_dispatch    = time.monotonic()
    silent_future = _dispatch_silent_face(orig_image, rid)

    # Haar: RESIZED frame -> bbox in resized coords
    t_haar  = time.monotonic()
    haar    = _haar_detect_single_face(small_image, rid)
    ...
    bbox       = haar["bbox"]            # resized-image coordinates
    face_count = haar["face_count"]

    # CV occlusion gate: stays on the RESIZED frame.
    # Its phone/edge/skin thresholds are calibrated for 480px, and the bbox is
    # already in resized coordinates — do NOT feed it the original frame.
    t_cv = time.monotonic()
    try:
        fv = _run_cv_visibility(small_image, bbox, face_count, rid)
    except Exception as e:
    ...
    # kprokofi: full-res ORIGINAL, with the Haar bbox scaled up to original coords.
    t_kprokofi      = time.monotonic()
    kprokofi_result = None
    if USE_KPROKOFI:
        try:
            kprokofi_bbox   = _scale_bbox(bbox, scale_to_orig, orig_w, orig_h)
            kprokofi_result = _run_kprokofi(orig_image, kprokofi_bbox, rid)
        except Exception as e:
    ...
    return {
        ...
        "antispoof_detail": {
            ...
            "timing": {
                ...
                # Haar + CV gate ran on these dims; the liveness models ran on
                # the original dims above.
                "processed_dims":   [small_image.shape[1], small_image.shape[0]],
                "model_input_dims": [orig_image.shape[1], orig_image.shape[0]],
                "scale_to_orig":    round(scale_to_orig, 4),
            },
        },
    }
